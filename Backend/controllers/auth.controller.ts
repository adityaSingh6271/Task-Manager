...
export const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existByEmail) return res.status(400).json({ error: "Email already registered" });

    const existByMobile = await prisma.user.findUnique({ where: { mobile } });
    if (existByMobile) return res.status(400).json({ error: "Mobile already registered" });

    const hashed = await bcrypt.hash(password, 10);

    const user = await prisma.user.create({
      data: { email, mobile, password: hashed, name: name ?? "" },
      select: { id: true, email: true, mobile: true, name: true, avatar: true, createdAt: true },
    });

    return res.status(201).json({ message: "Account created", user });
  } catch (err: any) {
    if (err?.code === "P2002") return res.status(400).json({ error: "Unique constraint failed" });
...
export const sendOtp = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.json({ message: "OTP generated", otp }); // For dev only
  } catch {
    return res.status(500).json({ error: "Internal server error" });
  }
};
...
export const verifyOtp = async (req: Request, res: Response) => {
  ...
  try {
    const record = await prisma.oTP.findFirst({ where: { mobile, otp }, orderBy: { createdAt: "desc" } });
